define(['dialog', 'tabbook', 'tabpage', 'item'], function(Dialog, TabBook, TabPage, Item) {
            ...
            this.enchantButton.off('click').on('click', function(event) {
                    ...
                    if(game && game.ready) {
                        ...
                        self.parent.confirm(enchantString, function(result) {
                            if(result && game.repairDialog.visible) {
                                game.client.sendStoreRepair(1, self.selectedInventory.getIndex()+6);
                                self.release();
                            }
                        });
                    }
                ...
                if(self.selectedEquipment) {
                    ...
                    if(game && game.ready) {
                        ...
                        self.parent.confirm(enchantString, function(result) {
                            if(result && game.repairDialog.visible) {
                                game.client.sendStoreRepair(2, self.selectedEquipment.getIndex());
                                self.release();
                            }
                        });
                    }
    ...
    var RepairDialog = Dialog.extend({
        init: function(game) {
